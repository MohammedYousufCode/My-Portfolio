...
export default function Hero({ profile }: { profile: Profile }) {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 pt-16 pb-12">
      <div className="hero-glow" />
      <div className="max-w-6xl w-full mx-auto">
        <motion.div variants={stagger} initial="hidden" animate="show"
          className="flex flex-col md:grid md:grid-cols-2 gap-8 md:gap-16 items-center">

          {/* Photo — shown first on mobile, right col on desktop */}
          <motion.div variants={item}
            className="flex justify-center md:order-last w-full">
          ...
          <div className="text-center md:text-left">
            ...
            <motion.p variants={item}
              className="body-text text-sm md:text-base mb-6 max-w-md mx-auto md:mx-0">
              {profile.bio}
            </motion.p>
            ...
            {/* Buttons */}
            <motion.div variants={item} className="flex flex-wrap gap-3 mb-8 justify-center md:justify-start">
              {profile.resume_url && (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          className="mt-10 md:mt-16 grid grid-cols-2 md:grid-cols-4 gap-px border border-gray-100 dark:border-white/5 rounded-xl overflow-hidden">
          {[
            ...
            <div key={label}
              className="bg-gray-50 dark:bg-dark-card px-4 py-4 text-center hover:bg-yellow-50 dark:hover:bg-white/5 transition-colors">
              <div className="font-playfair font-bold text-lg md:text-2xl shimmer-text">
                {value}{suffix}
              </div>
              <div className="font-mono text-xs text-gray-400 tracking-widest uppercase mt-1 leading-tight">{label}</div>
            </div>
          ))}
        </motion.div>
      </div>
